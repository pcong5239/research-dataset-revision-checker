...
1. A maintainer connects a supported wallet. For a fresh public run, create a public GitHub repository, make an initial commit, then add repository.json and copy its Raw HTTPS URL. Add landing.json in the next commit. Both files use a fresh dataset_id, matching version, license_id, release_ref and the initial commit_id; landing.json sets repository_url to exactly the repository.json Raw URL, not the normal repository page URL, and repository.json sets repository_owner and repository_name to the owner/repository path. Copy the two Raw HTTPS URLs into the form, using the repository.json Raw URL in the field labelled Canonical repository URL, with the expected version and license, then choose Register case.
